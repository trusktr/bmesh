...
export class BMesh {
     ...
     */
    static validateRadial(loop) {
        if (!loop.radialLink)
            return new Error('loop has no radial link');
        try {
            const links = [...loop.radialLink];
            if (links.length !== loop.edge.faceCount)
                return new Error('radial link length does not match face count: ' + links.length + ' !== ' + loop.edge.faceCount);
            let i = 0;
            for (const link of links) {
                const nextLink = links[(i + 1) % loop.edge.faceCount];
                if (nextLink.prev !== link)
                    return new Error('reverse link does not match forward link');
                ...
                if (link.loop.edge !== nextLink.loop.edge)
                    return new Error('each link loop must have the same edge');
     ...
     */
    static validateDisk(vertex, edge = null) {
        if (!vertex.diskLink) {
            if (vertex.edgeCount)
                throw new Error('vertex has no disk link but edge count is not 0');
            return null;
        }
        ...
        try {
            ...
            for (const link of links) {
                ...
                if (nextLink.prev !== link)
                    return new Error('reverse link does not match forward link');
                if (links.length !== 1 && link.edge === nextLink.edge)
                    return new Error('each link must have a different edge');
                if (!link.edge.hasVertex(vertex))
                    return new Error('each link edge should have the vertex of the link and the next link');
                if (edge && link.edge === edge)
                    includesEdge = true;
                i++;
            }
        }
        ...
        if (edge && !includesEdge)
            return new Error('disk link does not include edge');
        return null;
    }
}
