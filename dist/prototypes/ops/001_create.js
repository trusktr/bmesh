...
if (location.pathname.endsWith('001_create.html'))
    main();
export function drawMeshVertsEdges(mesh) {
    for (const v of mesh.vertices)
        Debug.pnt.addPoint(v.toArray(), 0x00ff00, 3);
    for (const e of mesh.edges)
        Debug.ln.addPoint(e.vertexA.toArray(), e.vertexB.toArray(), 0x00ffff);
}
export function drawFaceVertsEdges(f, lineColor = deeppink) {
    // const offset = Math.random()
    const offset = 0;
    for (const l of f.loop) {
        const pointSize = 5;
        Debug.pnt.addPoint(vec3.add(l.vertex.toArray(), [offset, offset, offset]), deeppink, pointSize);
        Debug.ln.addPoint(vec3.add(l.edge.vertexA.toArray(), [offset, offset, offset]), vec3.add(l.edge.vertexB.toArray(), [offset, offset, offset]), lineColor);
    }
}
export function drawVertEdges(v) {
...
export function drawFacePoint(face, color = yellow) {
    const pointSize = 7;
