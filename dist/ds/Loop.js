...
export class Loop {
    // #region MAIN
    vert;
    edge; // Start Edge, ver order doesn't matter in this constraint? [Loop.v,Loop.next.v] == [ Edge.v1, Edge.v2 ],
    face; // There will only be one face using an edge
    ...
    iterNext() {
        ...
        const next = () => {
            if (result.value !== undefined && loop == this)
                result.done = true;
            result.value = loop;
            loop = loop.next;
            return result;
