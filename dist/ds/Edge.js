...
export class DiskLink {
    next; // Use any to make it null without the requirement to have null checks
    prev; // Will only be set to null when deleting to help with garbage collection
}
export class Edge {
    // #region MAIN
    ...
    v1;
    v2;
    loop = null; // First loop, use to loop over all faces this edge is part of
    v1_disk = new DiskLink(); // Circular linked List of edges using V1 as origin
    v2_disk = new DiskLink(); // Circular linked List of edges using v2 as origin
    constructor(v1, v2) {
        if (v1 && v2) {
            this.v1 = v1;
            this.v2 = v2;
