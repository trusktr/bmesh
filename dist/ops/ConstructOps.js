import CoreOps from './CoreOps.js';
// #endregion
export class ConstructOps {
    ...
    static edgesFromVertsEnsure(bm, verts) {
        const rtn = [];
        let edge;
        let ii;
    ...
    static edgesSortWinding(v1, v2, edges, len, edges_sort, verts_sort) {
        ...
        verts_sort[0] = v_iter;
        do {
            v_iter = e_iter.getOtherVert(v_iter); // Find edge with the other vert
            isFound = false;
            for (let j = 0; j < edges.length; j++) {
    ...
    static faceCreateNgon(bm, v1, v2, edges, len) {
        const edges_sort = new Array(len);
        const verts_sort = new Array(len);
