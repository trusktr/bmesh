...
export class QueryOps {
    ...
    static edgeExists(v1, v2) {
        if (v1.edge !== null && v2.edge !== null) {
            let iter1 = v1.edge;
            let iter2 = v2.edge;
            do {
                if (iter1.vertExists(v2))
                    return iter1;
                if (iter2.vertExists(v1))
                    return iter2;
                iter1 = iter1.diskEdgeNext(v1);
                iter2 = iter2.diskEdgeNext(v2);
            } while (iter1 !== null && iter1 !== v1.edge && iter2 !== null && iter2 !== v2.edge);
        }
        return null;
    }
    ...
    static edgeFaceCountIsOver(e, n) {
        return !!(this.edgeFaceCountAtMost(e, n + 1) === n + 1);
    }
    ...
    static edgeFaceCountAtMost(e, count_max) {
        let count = 0;
        if (e.loop) {
            let l_iter = e.loop;
            do {
    ...
    static faceShareEdgeCount(f_a, f_b) {
        let l_iter = f_a.loop;
        let count = 0;
        do {
            if (this.edgeInFace(l_iter.edge, f_b))
                count++;
        } while ((l_iter = l_iter.next) != f_a.loop);
        return count;
    }
    // CUSTOM: Get the size of the radial linked list of a loop
    static loopRadialCount(l) {
        let cnt = 0;
        let iter = l;
        do {
            cnt++;
    ...
    static vertPairShareFaceCheck(v_a, v_b) {
        if (v_a.edge && v_b.edge) {
            ...
            do {
                if (eIter.loop) {
                    lIter = eIter.loop;
                    do {
                        if (this.vertInFace(v_b, lIter.face))
                            return true;
