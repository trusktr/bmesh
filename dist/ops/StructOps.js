...
export class StructOps {
    ...
    static diskEdgeRemove(e, v) {
        const d1 = e.getDiskFromVert(v);
        let d2;
        if (d1.prev) {
    ...
    static edgeVertSwap(e, v_dst, v_src) {
        // swap out loops
        if (e.loop) {
            let l_iter = e.loop;
            do {
                if (l_iter.vert == v_src)
                    l_iter.vert = v_dst;
                else if (l_iter.next.vert == v_src)
                    l_iter.next.vert = v_dst;
    ...
    static diskVertReplace(e, v_dst, v_src) {
    ...
    static diskVertSwap(e, v_dst, v_src) {
        if (e.v1 == v_src) {
            e.v1 = v_dst;
    ...
    static diskCountAtMost(v, count_max) {
        let count = 0;
        if (v.edge) {
            let e_iter = v.edge;
            do {
    ...
    static loopValidate(f) {
        ...
        for (i = 1, l_iter = l_first.next; i < len; i++, l_iter = l_iter.next) {
            if (l_iter.face != f || l_iter == l_first)
                return false;
        }
        if (l_iter != l_first)
            return false;
