...
export function Link<T extends AnyConstructor = typeof Empty>(BaseClass: T = Empty as T) {
	...
	return class Link extends BaseClass {
		...
		iterator(forward = true, checkCircular = true) {
			...
			type LinkIterator = Iterator<NonNullable<this['next']>> & { [Symbol.iterator](): LinkIterator }

			const iterator: LinkIterator = {
				[Symbol.iterator]: () => iterator,

				next: (): IteratorResult<Link> => {
					if (!link || (i !== 0 && link === this)) {
						if (!link && this.circular && checkCircular) throw new NonCircularError()
		...
		forEach(fn: (link: NonNullable<this['next']>) => boolean | void, forward = true, checkCircular = true) {
			...
			while (link) {
				...
				if (this.circular && checkCircular && !link) throw new NonCircularError()
				if (fn(l) === false) break
				if (link === this) break // circular done
			}
		}
