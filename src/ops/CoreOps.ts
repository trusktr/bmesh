...
export class CoreOps {
	...
	static joinFaceKillEdge(bm: BMesh, f1: Face, f2: Face, e: Edge): Face | null {
		...
		if (!((l_f1 = QueryOps.faceEdgeShareLoop(f1, e)) && (l_f2 = QueryOps.faceEdgeShareLoop(f2, e)))) return null
		...
		if (
			QueryOps.edgeInFace(l_f1.next.edge, f2) ||
	...
	// do_del : Delete Vert,
	static joinEdgeKillVert(
		bm: BMesh,
		e_kill: Edge,
		v_kill: Vertex,
		do_del = true,
		check_edge_exists = true,
		kill_degenerate_faces = true,
		kill_duplicate_faces = true,
	) {
